...
VideoDetailActivity::VideoDetailActivity(bilibili::Video video):video_data(std::move(video)){
    Logger::error("create VideoDetailActivity1: {}", video_data.bvid);
}

VideoDetailActivity::VideoDetailActivity(std::string bvid){
    video_data.bvid = std::move(bvid);
    Logger::error("create VideoDetailActivity2: {}", video_data.bvid);
}

void VideoDetailActivity::onContentAvailable() {
...
void VideoDetailActivity::setFullscreen(){
...
void VideoDetailActivity::exitFullscreen(){
...
void VideoDetailActivity::onVideoInfo(const bilibili::VideoDetailResult &result) {
...
void VideoDetailActivity::onVideoPageListInfo(const bilibili::VideoDetailPageListResult &result) {
    for(const auto& i : result){
        Logger::debug("cid:{} title:{}", i.cid, i.part);
    }
}

void VideoDetailActivity::onVideoPlayUrl(const bilibili::VideoUrlResult & result) {
...
bool VideoDetailActivity::isTranslucent() {
    return true;
}
