...
public:
    ...
    std::list<Node<K,T> >& getCacheList(){
        return cacheList;
    }
private:
    int capacity;
...
public:
    ...
    int getCache(const std::string& url){
...
ImageHelper* ImageHelper::into(brls::Image* image){
    ...
    cpr::GetCallback([this, image](cpr::Response r) {
        ...
        if (r.status_code != 200 || r.downloaded_bytes == 0 || this->isCancel){
            ...
            brls::Logger::debug("load pic to image1 {} {}", (size_t)image, image->describe());
            std::uniform_real_distribution<double> u(10, 100);
            brls::Threading::delay(u(random_engine), [this, image, r](){
                image->setImageFromMem((unsigned char *)r.text.c_str(), (size_t)r.downloaded_bytes);
                if(image->getTexture() > 0)
                    TextureCache::instance().addCache(this->imageUrl, image->getTexture());
                this->available = true;
                image->ptrUnlock();
            });
        }
...
brls::Image* ImageHelper::getImageView(){
