...
static YGSize textBoxMeasureFunc(YGNodeRef node, float width,
    ...
    std::shared_ptr<NVGtextRow> rows(new NVGtextRow[maxRows + 1], [](NVGtextRow* p){delete []p;});
...
void TextBox::draw(NVGcontext* vg, float x, float y, float width, float height,
    ...
    std::shared_ptr<NVGtextRow> rows(new NVGtextRow[maxRows + 2], [](NVGtextRow* p){delete []p;});
