...
void HomeBangumiRequest::requestBangumiList(int is_refresh, std::string cursor) {
...
DataSourcePGCVideoList::DataSourcePGCVideoList(bilibili::PGCModuleResult result): videoList(result){
    if(!result.url.empty()){
        showMore = true;
    }
}
...
void DataSourcePGCVideoList::clearData(){
