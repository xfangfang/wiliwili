...
public:
    ...
    void getLoginUrl(){
        ASYNC_RETAIN
        bilibili::BilibiliClient::get_login_url([ASYNC_TOKEN](const std::string& url, const std::string& key){
            ASYNC_RELEASE
            this->oauthKey = key;
            this->login_url = url;
    ...
    void checkLogin(){
        ...
        ASYNC_RETAIN
        bilibili::BilibiliClient::get_login_info(this->oauthKey, [ASYNC_TOKEN](bilibili::LoginInfo info){
            ...
            switch(info){
                ...
                case bilibili::LoginInfo::OAUTH_KEY_ERROR:
                    this->onLoginError();
                    this->onLoginStateChange("二维码失效，请重新进入对话框");
                    break;
