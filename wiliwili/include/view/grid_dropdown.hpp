...
{
public:
    DataSourceDropdown(std::vector<std::string> result, GridDropdown* view):data(result), dropdown(view){}
...
public:
    ...
    GridDropdown(std::string title, std::vector<std::string> values, ValueSelectedEvent::Callback cb,
                 int selected = 0): values(values), cb(cb), selected(selected)
    {
        ...
        float height = dataSource->getItemCount() * style["brls/dropdown/listItemHeight"]
                       + header->getHeight()
